...
function openActionModal(type, productId){
    ...
    /* ===== DELETE ===== */
    if(type === 'delete'){
        title.innerHTML = 'Удаление товара';
        content.innerHTML = '<div>Удалить товар <b>' + escapeHtml(product.name) + '</b> ?</div>';
    }

    /* ===== REDUCE ===== */
    if(type === 'reduce'){
        title.innerHTML = 'Списание товара';
        content.innerHTML = '<div>Товар: <b>' + escapeHtml(product.name) + '</b></div>' +
            '<input type="number" id="reduceAmountInput" placeholder="Сколько списать">';
    }

    /* ===== ADD ===== */
    if(type === 'add'){
        title.innerHTML = 'Пополнение товара';
        content.innerHTML = '<div>Товар: <b>' + escapeHtml(product.name) + '</b></div>' +
...
async function confirmAction(){
    ...
    if(currentAction === 'reduce'){
        const amount = Number(document.getElementById('reduceAmountInput').value);
        if(!amount || amount <= 0){
            showStatus('Введите количество', 'error');
            return;
        }
        await reduceQuantity(currentProductId, amount);
        closeActionModal();
        return;
    }
        ...
        try{
            const response = await fetch(APP_CONFIG.API_URL + '/' + currentProductId, {
                ...
                body: JSON.stringify({
                    ...
                    medicineSeries: product.medicineSeries,
                    medicineManufacturer: product.medicineManufacturer,
                    medicineDosage: product.medicineDosage,
            ...
            if(data.success){
                ...
                showStatus('Товар пополнен', 'success');
                closeActionModal();
                loadProducts();
            } else {
...
console.log('✅ products-actions.js loaded, functions exported');